  ...
  const handleSubmit = async (e) => {
    try {
      if (mode === "update") {
        ...
        await updatePlanet(planetState.name, JSON.stringify(updatedFields));
      } else {
        const { _id, ...createFields } = planetState;
        await createPlanet(createFields);
      }
      e.preventDefault();
      navigate("/");
    ...
    <form className="formContainer">
      ...
      <label>
        ...
        <Input
          ...
          htmlType="number"
          required
          value={planetState.orbitalPeriod}
          onChange={handleInputChange}
        />
      </label>
      <label>
